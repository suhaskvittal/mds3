...
#define DEFAULT_MEM_DELAY    5000000

#ifndef MCORE_DO_WRITEBACKS
...
void mcore_cycle (MCore *c)
{
  ...
  for(uns ii = 0; ii < CORE_WIDTH; ii++)
  {
    ...
    if(c->inst_num >= c->trace_inst_num)
    {
      ...
      if(c->trace_wb)
      {
	        if (!mcache_mark_dirty(c->l3cache, orig_lineaddr)) ++c->dropped_writes;
      }
      else // READ
      {
    ...
    if(mem_access)
    {
      DBGMSGC(c->id, "CORE-RDSEND for CoreID: %u ROBID: %u InstNum: %llu Cycle: %llu\n", c->id, myrobid, c->inst_num, c->cycle);
...
void mcore_print_stats(MCore *c)
{
  ...
  printf("\n%s_DROPPED_WR   \t : %llu", header, c->dropped_writes);
...
uns mcore_rob_retire(MCore *c)
{
  ...
  for(uns ii = 0; ii < CORE_WIDTH; ii++)
  {
    if( c->rob.entries[c->rob.ptr].ready_time <= c->cycle)
    {
      ...
      {
        if (!(c->sleep && c->rob.entries[c->rob.ptr].inst_num == c->sleep_inst_num))
        {
	        DIEMSG("LOST-PACKET: Core: %u ROBID: %u InstNum: %llu Cycle: %llu, isSLeep: %u, sleep_instnum: %llu\n", c->id, c->rob.ptr,  c->rob.entries[c->rob.ptr].inst_num, c->cycle, c->sleep, c->sleep_inst_num);
        }
      }
