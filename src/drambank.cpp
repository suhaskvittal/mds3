...
uns64   dram_bank_service(DRAM_Bank *b,  DRAM_ReqType type, uns64 rowid, uns64 req_birthtime){
  ...
  if(new_act){
    ...
    //--- RFM scheduling as at 5 times RFMTH (equivalent for four entry DMQ)
    if( (DRAM_RFM_POLICY>0) && (b->RAA>=3*RFM_TH)){
    ...
    // ---- para DRFM modeling
    if(MCPARA_DRFM_PERIOD){
      double randval = (double)(rand()%(1<<20))/(double)(1<<20);
      double para_p = 1/(double)(MCPARA_DRFM_PERIOD);

      if( randval < para_p){
	uns para_delay = tPRE+tDRFM;
	b->row_valid = FALSE; // DRFM closes the row
	b->sleep_cycle += para_delay;
      }
    }
