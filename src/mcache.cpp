...
Flag
mcache_probe(MCache *c, Addr addr)
{
    Addr  tag  = addr; // full tags
    uns   set  = mcache_get_index(c,addr);
    uns   start = set * c->assocs;
...
Flag    
mcache_invalidate(MCache *c, Addr addr)
{
  Addr  tag  = addr; // full tags
  uns   set  = mcache_get_index(c,addr);
  uns   start = set * c->assocs;
...
uns mcache_find_victim (MCache *c, uns set)
{
  ...
  switch(c->repl_policy)
  {
    ...
    case REPL_LRU_WBT:
      return mcache_find_victim_lru_wbt(c, set);
    default: 
      exit(1);
  }
...
uns
mcache_find_victim_lru_wbt(MCache* c, uns set) {
    uns start = set   * c->assocs;    
    uns end   = start + c->assocs;    
    uns lowest = start;
    uns ii;
    ...
    for (ii = start; ii < end; ii++) {
        ...
        if (!found || c->entries[ii].last_access < c->entries[lowest].last_access) {
            if (!c->entries[ii].dirty || c->wb_in_2k_epoch < LRU_WB_TTH) { 
                lowest = ii;
                found = true;
            }
        }
    ...
    uns lruv = mcache_find_victim_lru(c,set);
    if (!found) {
        lowest = lruv;
        ++c->s_wb_forced;
    } else {
        if (lruv != lowest && c->entries[lruv].dirty && !c->entries[lowest].dirty) {
            ++c->s_wb_avoided;
        }
    }
    if (c->entries[lowest].dirty) ++c->wb_in_2k_epoch;
    return lowest;
}
...
void  mcache_print_stats(MCache *c, char *header)
{
  ...
  printf("\n%s_WB_AVOIDED   \t : %llu", header, c->s_wb_avoided);
  printf("\n%s_WB_FORCED    \t : %llu", header, c->s_wb_forced);
  printf("\n");
}
