    ...
    : config_(config), channel_id_(channel_id)
{
    ...
    InitStat("num_pre2_cmds", "counter", "Number of PRE w CTR UPDATE (PRE2) commands");
    InitStat("num_ondemand_pres", "counter", "Number of ondemend PRE commands");
    InitStat("num_refab_cmds", "counter", "Number of REFab commands");
    InitStat("num_refsb_cmds", "counter", "Number of REFsb commands");
...
void SimpleStats::PrintFinalStats(bool stdout) {
    UpdateFinalStats();
    ...
    if (config_.output_level >= 1) {
        ...
        ss << GetTextHeader(true);
        for (const auto& it : print_pairs_) {
            PrintStatText(ss, it.first, it.second,
                          header_descs_[it.first]);
        }
        if (stdout)
        {
...
void SimpleStats::UpdateCounters() {
    // std::cout << "[Hritvik] UpdateCounters" << std::endl;
...
void SimpleStats::UpdateEpochStats() {
    ...
    doubles_["refab_energy"] =
        epoch_counters_["num_refab_cmds"] * config_.ref_energy_inc;
...
void SimpleStats::UpdateFinalStats() {
    ...
    doubles_["refab_energy"] = counters_["num_refab_cmds"] * config_.ref_energy_inc;
    doubles_["refb_energy"] =
